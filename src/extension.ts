...
async function runLoader(targets: string[], recursive: boolean) {
  ...
  if (!script) {
    vscode.window.showErrorMessage('PyMOL Loader: wrapperScript path is not set.');
    return;
  }
  try { await lstat(script); } catch {
    vscode.window.showErrorMessage(`PyMOL Loader: wrapperScript not found: ${script}`);
    return;
  }

  const args = [script, '--port', String(port)];
  if (recursive) args.push('--recursive');           // ✅ restore recursive
